...
void MeasureSettings::onMeasurementPropertyChanged(QStandardItem *item)
{
    ...
    if (item->column() == 1 &&m_emitActivated &&
            m_ui->button_measDisplayAll_2->isChecked()) {
        m_ui->button_measDisplayAll_2->setChecked(false);
    }
...
void MeasureSettings::on_button_measDisplayAll_2_toggled(bool checked)
{
    m_ui->button_measDeleteAll_2->setEnabled(!checked);
...
void MeasureSettings::setSelectedChannel(int chnIdx)
{
    if (m_selectedChannel != chnIdx) {
        ...
        if (m_ui->button_measDisplayAll_2->isChecked()){
            disableDisplayAllMeasurements();
            displayAllMeasurements();
        }
...
void MeasureSettings::activateDisplayAll(){
    ...
    m_ui->button_measDisplayAll_2->setChecked(m_enableDisplayAll);
}
